...
# =========================================================
# 6. EXPORTACIÓN A EXCEL Y RESET DE FORMULARIO
# =========================================================
def si_no(valor):
    """Convierte booleanos de Streamlit a texto claro para Excel."""
    return "SI" if bool(valor) else "NO"


def binario(valor):
    """Convierte booleanos a 1/0 para que Excel guarde dato binario real."""
...
def preparar_registro_gs(datos, res):
    """
    Genera un registro PLANO para Excel.
...
def crear_excel_individual(datos, res):
    ...
    df_resumen = pd.DataFrame([
        {"Campo": "Paciente", "Valor": datos.get("nombre", "")},
        {"Campo": "Fecha", "Valor": datos.get("fecha", "")},
        {"Campo": "Médico", "Valor": datos.get("medico", "")},
        ...
        {"Campo": "Presión de pulso 24 h", "Valor": res.get("pp_24h", "")},
        {"Campo": "Meta PA", "Valor": res.get("meta_pa", "")},
    ])
    df_tensional = pd.DataFrame([
        {"Método": "Consultorio", "PAS": datos.get("c_pas"), "PAD": datos.get("c_pad"), "PP": datos.get("c_pas") - datos.get("c_pad")},
        {"Método": "MDPA", "PAS": datos.get("m_pas"), "PAD": datos.get("m_pad"), "PP": datos.get("m_pas") - datos.get("m_pad")},
        {"Método": "MAPA 24 horas", "PAS": datos.get("m24_pas"), "PAD": datos.get("m24_pad"), "PP": datos.get("m24_pas") - datos.get("m24_pad")},
        {"Método": "MAPA diurno", "PAS": datos.get("mdia_pas"), "PAD": datos.get("mdia_pad"), "PP": datos.get("mdia_pas") - datos.get("mdia_pad")},
        {"Método": "MAPA nocturno", "PAS": datos.get("mnoc_pas"), "PAD": datos.get("mnoc_pad"), "PP": datos.get("mnoc_pas") - datos.get("mnoc_pad")},
    ])
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        df_resumen.to_excel(writer, index=False, sheet_name="Resumen")
        df_tensional.to_excel(writer, index=False, sheet_name="Valores")
        df_registro.to_excel(writer, index=False, sheet_name="Registro completo")
    output.seek(0)
    nombre_seguro = (datos.get("nombre") or "Paciente").replace(" ", "_").replace("/", "-")
...
def mostrar_interfaz():
    # La app usa Excel local como almacenamiento y descarga.
        ...
        with col_b1:
            if st.button("💾 Guardar y Generar Archivos", use_container_width=True):
                if not nombre:
                    ...
                    st.download_button(
                        label="📊 Descargar Excel del Paciente",
                        data=excel_bytes,
